...
// Cached *values* (once resolved) plus in-flight *promises* (while resolving).
// The promise cache is what matters for performance: without it, a page that
// fires several getAll() calls in a Promise.all (dashboard, broadcast, …) would
// each independently kick off their own "is scoping available" / "who is the
// primary election" round trips — up to a dozen redundant requests on every
// cold load, which is what made pages feel slow. With the promise memoized,
// concurrent callers share the single in-flight request.
let _scopeAvailCache: boolean | null = null
let _scopeAvailPromise: Promise<boolean> | null = null
let _primaryIdCache: string | null | undefined = undefined
let _primaryIdPromise: Promise<string | null> | null = null

export async function scopingAvailable(): Promise<boolean> {
  if (_scopeAvailCache !== null) return _scopeAvailCache
  if (!_scopeAvailPromise) {
    _scopeAvailPromise = (async () => {
      // Scoping needs BOTH the elections table AND an election_id column on the
      // data tables. If either is missing (e.g. a partially-applied migration)
      // we disable scoping so data is shown as a single election rather than
      // disappearing.
      const e = await supabase.from("elections").select("id").limit(1)
      if (e.error) { _scopeAvailCache = false; return false }
      const col = await supabase.from("votes").select("election_id").limit(1)
      _scopeAvailCache = !col.error
      return _scopeAvailCache
    })()
  }
  return _scopeAvailPromise
}

// Force scoping off for the rest of the session (used when a scoped query
// errors out, e.g. a column genuinely isn't there) — never leaves data hidden.
function disableScoping() {
  _scopeAvailCache = false
  _scopeAvailPromise = Promise.resolve(false)
}
...
async function primaryElectionId(): Promise<string | null> {
  if (_primaryIdCache !== undefined) return _primaryIdCache
  if (!_primaryIdPromise) {
    _primaryIdPromise = (async () => {
      const { data } = await supabase.from("elections").select("id").order("created_at", { ascending: true }).limit(1).maybeSingle()
      _primaryIdCache = data?.id ?? null
      return _primaryIdCache
    })()
  }
  return _primaryIdPromise
}

// Elections were created/removed — the "who is primary" answer could change.
function resetElectionCaches() {
  _primaryIdCache = undefined
  _primaryIdPromise = null
}
...
//
// Note: this trusts the given/stored id rather than re-validating it against a
// list of known elections on every call. That whitelist check used to be cached
// too, and a stale cache (e.g. a page left open across an election being
// created) meant an id created *after* the cache was built looked "invalid" —
// silently falling back to unscoped (primary) data. That's a much worse failure
// mode than the one it was guarding against: a genuinely stale/deleted id just
// matches zero rows (handled explicitly, see scopeFilter), which reads as
// "no data" rather than "someone else's election's data".
...
export const electionsDb = {
  ...
  create: async (input: { name: string; organization?: string; term?: string; slug?: string }): Promise<Election | null> => {
    ...
    resetElectionCaches() // a new election could become "primary" if created before... it never does, but keep this cheap & correct
  ...
  remove: async (id: string): Promise<boolean> => {
    ...
    resetElectionCaches()
    // Don't leave a dangling reference to a deleted election in this browser.
    if (getCurrentElectionId() === id) setCurrentElectionId(null)
